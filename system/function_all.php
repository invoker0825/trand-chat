...
function removeAllAction($user){
	global $mysqli;
	$mysqli->query("UPDATE boom_users SET user_kick = 0, user_mute = 0, user_rmute = , user_mmute = 0, user_banned = 0 WHERE user_id = '{$user['user_id']}'");
...
function currentUserRoom($user){
	if(inChat($user) && isVisible($user) && canViewRoom()){
		$room = roomDetails($user['user_roomid']);
		if(!empty($room)){
...
function boomInsertUser($pro, $type = 0){
	...
	if(useRegmute()){
		$regmute = calMinutesUp($setting['reg_mute']);
	}
...
function nextClean(){
	...
	if($c['last_clean'] <= calMinutes(5)){
		$mysqli->query("UPDATE boom_clean SET last_clean = '" . time() . "' WHERE id = 1");
		return true;
	}
}
...
function botPostChat($id, $room, $content, $custom = array()){
	...
	$mysqli->query("
	INSERT INTO `boom_chat` (post_date, user_id, post_message, post_roomid, type, log_rank, tcolor) VALUES ('" . time() . "', '$id', '$content', '$room', '{$post['type']}', '{$post['rank']}', '{$post['color']}')");
	chatAction($room);	
	return true;
}
...
function createModal($c, $t, $s = 400){
	return [
		'content'=> $c,
		'type'=> $t,
		'size'=> $s,
