...
function canViewGold(){
	global $setting;
	if(useGold() && boomAllow($setting['can_vgold'])){
		return true;
	}
}
...
function useGold(){
	global $setting;
	if($setting['use_gold'] > 0){
		return true;
	}
}
function canGold(){
	global $setting;
	if(boomAllow($setting['allow_gold'])){
		return true;
	}
}
function canReceiveGold($user){
	global $setting;
	if(!isBot($user) && userBoomAllow($user, $setting['can_rgold'])){
		return true;
	}
}
function canShareGold($user){
	global $setting;
	if(!useGold() || isBot($user) || ignored($user) || ignoring($user) || !userBoomAllow($user, $setting['can_rgold'])){
		return false;
	}
	if(boomAllow($setting['can_sgold'])){
		return true;
	}
}
...
function fileFlood(){
	if(1 == 1){
		...
		if(!isset($_SESSION[BOOM_PREFIX .'ftime']) || $_SESSION[BOOM_PREFIX . 'ftime'] < (time() - 15)){
			$_SESSION[BOOM_PREFIX . 'ftime'] = time();
			$_SESSION[BOOM_PREFIX . 'fcount'] = 1;
			return false;
		}
		else if($_SESSION[BOOM_PREFIX . 'fcount'] >= 30){
			$_SESSION[BOOM_PREFIX . 'fignore'] = (time() + 3600);
			return true;
		}
...
function userChatDetails($id){
	...
	SELECT
	user_id, user_name, user_password, user_join, last_action, user_language, user_timezone, user_status, user_rank, user_roomid, session_id, pcount,
	pdel, pdeltime, user_news, user_ghost, user_mute, user_rmute, user_mmute, user_pmute, user_banned, user_kick, warn_msg, user_role, user_action, room_mute, naction, user_gold, last_gold,
