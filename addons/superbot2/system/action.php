...
function addSuperbotQuestion(){
	...
	if($query != ''){
		$mysqli->query("INSERT INTO superbot2_data (superbot2_question, superbot2_answer) VALUES $query");
		return 1;
	}
	else {
		return 0;
...
function findSuperbot(){
	...
	$list_bot = '';
	$search_answer = $mysqli->query("SELECT * FROM superbot2_data WHERE superbot2_answer LIKE '%$search%' OR superbot2_question LIKE '%$search%' ORDER BY superbot2_answer LIMIT 250");
	if( $search_answer->num_rows > 0){
		if($search_answer->num_rows > 0){
			while($ra = $search_answer->fetch_array()){
				$list_bot .= boomAddonsTemplate('../addons/superbot2/system/template/superbot2_data', $ra);			
			}
		}
		return $list_bot;
	}
...
if(isset($_POST['question'], $_POST['answer1'], $_POST['answer2'], $_POST['answer3'], $_POST['answer4'], $_POST['answer5'])){
	...
	$mysqli->query("DELETE FROM superbot2_data WHERE id = '$sbdelete'");
	echo 2452432;
}
else if(isset($_POST['set_superbot_access']) && canManageAddons()){
	$superbot_access = escape($_POST['set_superbot_access'], true);
	$mysqli->query("UPDATE boom_addons SET addons_access = '$superbot_access'WHERE addons = 'superbot2'");
	redisUpdateAddons('superbot2');
	echo 5;
	die();
}
else {
