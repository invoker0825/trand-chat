...
if ($max_room->num_rows > 0){
    $room = $max_room->fetch_assoc();
    $room_id = $room["room_id"];
    $user_list = $mysqli->query("SELECT action_user from boom_room_action where action_room=$room_id AND action_blocked>0");
    if ($user_list->num_rows > 0) {
        while ($user = $user_list->fetch_assoc()){
            $user_ids[] = $user["action_user"];
        }
    }
}
$ids = implode(",", $user_ids);
$data_list = $mysqli->query("
	SELECT user_id, user_name, user_color, user_font, user_rank, user_dj, user_onair, user_join, user_sex, user_age, user_tumb, user_cover, user_status, country,
...
if ($data_list && $data_list->num_rows > 0){
	while ($list = $data_list->fetch_assoc()){
		if($list['user_dj'] == 1 && $list['user_onair'] == 1){
			$onair_user .= createUserlist($list, false, true);
			$onair_count++;
		}
		else {
			if($lazy_state < $lazy_min){
				$online_user .= createUserlist($list, false, true);
			}
			else {
				$online_user .= createUserlist($list, true, true);
			}
			$online_count++;
			$lazy_state++;
		}
...
if($setting['max_offcount'] > 0){
	if($offline_list->num_rows > 0){
		while($offlist = $offline_list->fetch_assoc()){
			if($lazy_state < $lazy_min){
				$offline_user .= createUserlist($offlist, false, true);
				$lazy_state++;
			}
			else {
				$offline_user .= createUserlist($offlist, true, true);
			}
		}
	}
}
