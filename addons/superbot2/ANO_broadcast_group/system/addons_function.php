...
function listLive(){
	...
	$time = time();
	$lives_list = '';
	$live_list = $mysqli->query("SELECT * FROM boom_users WHERE user_lives > '$time'");
...
function userLazyCustom($user,$type){
	...
	if($type == 1){
		...
		$click = '<button onclick="sendRequestLive('.$user['user_id'].');" class="reg_button ok_btn"><i class="fa fa-paper-plane"></i> Join</button>';
	}
