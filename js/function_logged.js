...
appendRooms = function(){
	let visitedRooms = JSON.parse(localStorage.getItem("visitedRooms", "[]"));
	if (!visitedRooms)
		visitedRooms = [user_room];
	let innerHTML = "<div class='header-rooms'>";
	allRooms.filter(room => visitedRooms.find(idx => idx==room.room_id)).forEach(room => {
		const ask = room.password !== '';
		innerHTML += `
			<div class='header-room ${room.room_id==user_room?'active':''}' onclick="switchRoom(${room.room_id}, ${ask}, ${room.access});">
				${room.room_name}
				<span class="notification d-none" id="room-unread-${room.room_id}">0</span>
			</div>
		`;
	});
	innerHTML += "</div>";
	$("#empty_top_mob").html(innerHTML);
}
switchRoom = function (room, pass, rank) {
	console.log(room, pass, rank, user_room)
	if (insideChat()) {
		if (room == user_room) {
...
unblockUser = function(user_id){
	$.post("system/action/action.php", {
		take_action: "creator_room_unblock",
		target: user_id,
		room: user_room
	}, function(res) {
		$(`[data-i="u${user_id}"]`).remove();
	})
}
...
$(document).ready(function () {
	...
	$(document).on('click', '.avunblock', function(){
		var id = $(this).attr('data');
		closeTrigger();
		unblockUser(id);
	});
